...
const PuzzleForm = () => {
  const { mutate: puzzleMutate } = usePuzzle("4");
      ...
      <div className="flex flex-col gap-4 p-8 rounded-lg bg-gradient-to-br from-indigo-100 to-purple-100 min-h-screen">
        <div className="max-w-3xl mx-auto w-full">
          <div className="text-center text-lg text-gray-800 bg-white/90 backdrop-blur-sm rounded-xl shadow-xl p-6 border border-indigo-200">
            در مسیر حل پرونده عکسی را یافتیم ولی متاسفانه این عکس تکه تکه شده
            ،نیاز است این عکس را با جابه جایی ترمیم کنید.(پازل را تکمیل کنید)
          </div>
            ...
            <JigsawPuzzle
              ...
              onSolved={() => {
                puzzleMutate(
                  ...
                  {
                    onSuccess: () => {
                      toast.success("تبریک! شما با موفقیت پیتسو را حل کردید");
                      navigate("/missions");
                    },
                  }
                );
